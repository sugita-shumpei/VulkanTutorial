...
private:
	...
	bool isDeviceSuitable(VkPhysicalDevice physDev)
	{
		...
		if (!swapChainSupport.formats.empty() && !swapChainSupport.presentModes.empty() && supportedFeatures.samplerAnisotropy) {
			return true;
		}
		else {
			return false;
	...
	void createDescriptorSetLayout() {
		...
		layoutInfo.bindingCount = std::size(setLayoutBindings);
		layoutInfo.pBindings = std::data(setLayoutBindings);
	...
	void createTextureImage()
	{
		...
		stbi_uc* pixels = stbi_load(ASSETS_ROOT_DIR "/texture.jpg", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
	...
	void createDescriptorPool() {
		...
		poolInfo.maxSets = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT) * poolInfo.poolSizeCount;
	...
	void createDescriptorSets() {
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
			...
			vkUpdateDescriptorSets(device, std::size(descriptorWrites), std::data(descriptorWrites), 0, nullptr);
		}
